...
## Saved-history pagination - 2026-09-18

Confirmed working: history displays ten entries per page with Newer/Older controls. API validates offset >= 0 and limit 1..100 (default 20), preserving owner checks and newest-first ordering. Refresh/save/delete returns to newest history.

Verified: 79 backend tests, 7 frontend tests, lint/build and normal/demo Chrome workflows. API covers page boundaries, complete ordering, empty pages, bad parameters and ownership; browser covers eleven snapshots across two pages and disabled final-page navigation. No blockers or schema changes. Changes uncommitted per request; remote CI awaits future commit/push. Next suggested feature: search saved comparisons by title.

## Saved comparison deletion ? 2026-09-18

? Confirmed working: owners can permanently delete individual saved snapshots after in-app confirmation. Resume text, PDFs and other snapshots remain unchanged. Missing authentication returns 401; foreign/missing snapshots return 404; success returns empty 204.

Verification: 78 backend tests, 7 frontend tests, lint, production build, and normal/demo Chrome workflows pass. Browser checks cover cancellation, network failure/retry and persisted removal after refresh. Synthetic data only; no personal snapshots deleted. No schema changes for deletion.

Next suggested checkpoint: saved-history pagination as the list grows. No blockers. Remote CI remains pending a future commit/push; all changes remain uncommitted as requested.
...
User selected feature 3. Confirmed working: separate reviewed score uses only explicit user Required choices, detects confirmed terms from existing resume matches, reports denominator/missing terms and unreviewed/uncertain counts. Null until at least one confirmation and readable resume text; zero means confirmed requirements have no detected matches. Original keyword score unchanged.

Verified 7 frontend tests, lint, normal/demo builds and both Chrome workflows, including 100% subset explanation, reset and downloaded v2 JSON. No backend/database changes; prior 74 backend tests retained. Choices remain transient. Next: review/commit this feature checkpoint and verify remote CI; no new remote run yet. Changes uncommitted.
